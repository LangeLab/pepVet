...
plot_proteome_overview <- function(batch, title = NULL) {
  ...
  pb <- ggplot2::ggplot(comp_df, ggplot2::aes(y = label, x = score)) +
    ...
    ggplot2::labs(
      ...
      subtitle = "Median component score across all proteins\n\u00b7  Color = one digest quality dimension",
      x = "Median score",
      y = NULL
    ) +
    .pepvet_theme()
  ...
  if (length(present_flags) > 0L) {
    ...
    flag_long$flag     <- factor(
      flag_long$flag, levels = flag_labels[ordered_flags])
    flag_long$category <- factor(
      flag_long$category, levels = c("Not flagged", "Flagged"))
...
#'   by descending Good%, and the top-performing enzyme is marked with a \u2605.
...
#'   verdict.
#' - **(C) Component heatmap:** median component scores in an enzyme \u00d7 component
#'   grid, filled by the verdict gradient (red \u2192 amber \u2192 green).  Reveals which
#'   digest quality dimension differentiates the enzymes.
#' - **(D) Per-protein win rate:** bar chart showing the proportion of proteins
#'   for which each enzyme achieves the highest composite score.  The \u2605 enzyme
...
plot_batch_comparison <- function(comparison, title = NULL) {
  ...
  enz_long$verdict <- factor(
    enz_long$verdict, levels = c("Poor", "Moderate", "Good"))
  ...
  win_counts <- tabulate(factor(best_for, levels = enz_order))
  win_df <- data.frame(
    enzyme   = enz_order,
    n_wins   = win_counts,
