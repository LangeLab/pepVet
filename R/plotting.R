...
.panel_coverage <- function(peps, protein_length, length_range) {
  ...
  pct_cov <- round(100 * sum(covered) / protein_length, 1)

  # Find gap runs (contiguous uncovered regions)
  rl        <- rle(covered)
  ...
  # Label: valid peptides whose midpoint doesn't collide with neighbors
  # Show length for peptides >= 8 aa (enough space for a 2-digit label)
