## scratch/build-vignette-data.R
## Compile all comparison data into vignette-ready RDS datasets.
## Run after fetch-external-data.R, compare-presets.R, compare-peptideranger.R

pkgload::load_all(quiet=TRUE)

data_dir <- "scratch/comparison-data"
out_dir  <- "vignettes"
...
## 1. Load raw CSV data
##
pepvet_raw   <- read.csv(file.path(data_dir, "pepvet-bsa-trypsin.csv"),  stringsAsFactors=FALSE)
msdigest_raw <- read.csv(file.path(data_dir, "tool-compare-msdigest.csv"), stringsAsFactors=FALSE)
expasy_raw   <- read.csv(file.path(data_dir, "tool-compare-expasy.csv"),  stringsAsFactors=FALSE)
presets      <- read.csv(file.path(data_dir, "tool-compare-presets.csv"), stringsAsFactors=FALSE)
pr_summary   <- read.csv(file.path(data_dir, "tool-compare-peptideranger.csv"), stringsAsFactors=FALSE)
pr_peptides  <- read.csv(file.path(data_dir, "peptideranger-all-peptides.csv"), stringsAsFactors=FALSE)
...
cor_pep_msd <- cor(master$mz_mh_plus[master$in_msdigest],
                   master$msdigest_mz_mi[master$in_msdigest], use="complete.obs")
cor_pep_exp <- cor(master$mz_mh_plus[master$in_expasy],
                   master$expasy_mass[master$in_expasy], use="complete.obs")

mass_corr <- data.frame(
  tool_pair=c("pepVet vs MS-Digest", "pepVet vs ExPASy", "MS-Digest vs ExPASy"),
