## scratch/fetch-external-data.R
## Parse raw tool outputs from MS-Digest and ExPASy PeptideMass into clean CSVs.

library(tibble)

out_dir <- file.path("scratch", "comparison-data")
dir.create(out_dir, showWarnings=FALSE, recursive=TRUE)

##
## 1. MS-Digest
##
raw_msd <- "scratch/tool-data/msdigest-bsa-trypsin.csv"
if (file.exists(raw_msd)) {
  msd <- read.delim(raw_msd, skip=1, header=FALSE, sep="\t",
                     quote="", strip.white=TRUE)
  names(msd) <- c("num", "mz_mi", "mz_av", "modifications", "start", "end",
  ...
  write.csv(msd_unique, file.path(out_dir, "tool-compare-msdigest.csv"),
            row.names=FALSE)
  cat(sprintf("MS-Digest: %d rows (%d unique peptides)\n", nrow(msd),
              nrow(msd_unique)))
} else {
...
##
raw_expasy <- "scratch/tool-data/expasy-bsa-trypsin.html"
if (file.exists(raw_expasy)) {
  ...
  expasy_list <- expasy_list[!sapply(expasy_list, is.null)]
