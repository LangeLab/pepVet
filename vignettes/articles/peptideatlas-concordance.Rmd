...
description: >
  Tests whether pepVet-valid peptides have higher PeptideAtlas observation rates than invalid peptides and examines bounded parameter and threshold grids.
output:
...
1. **Observation-rate difference**: for each protein, compute the observation rate among pepVet-valid and pepVet-invalid peptides. Report the mean difference across proteins with a bootstrap interval.
...
| Metric | Value |
|--------|-------|
| Observation rate (pepVet-valid peptides) | `r sprintf("%.1f%%", 100 * valid_rate)` |
| Observation rate (pepVet-invalid peptides) | `r sprintf("%.1f%%", 100 * invalid_rate)` |
...
| AHP weight vector | Not tested | A separate sensitivity analysis measures stability only under its specified perturbation model |
