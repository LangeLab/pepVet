...
`recommend_enzyme()` runs `compare_digests()` internally and returns the name of the highest-scoring enzyme as a character string. If multiple enzymes tie on the composite score, the function returns all tied names in alphabetical order.
...
`digest_report()` prints a styled summary for an `evaluate_digest()` result or a ranked table for a `compare_digests()` result. It returns its input invisibly.
