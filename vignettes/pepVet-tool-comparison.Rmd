...
Six tools in this comparison simulate or evaluate proteolytic digests. The analysis records peptide overlap for one BSA trypsin configuration, then compares scoring, filtering, ranking, structural context, and programmatic access.

This vignette is the companion to the [getting-started guide](pepVet-introduction.html). It compares pepVet against five other tools on a panel of 5 proteins and 5 enzymes.

## Comparison basis

The overlap analysis establishes the common digestion output before the capability review separates the tools by purpose and interface.
...
MS-Digest (Baker, Clauser, and Burlingame, UCSF) returns a tab-delimited table of peptide masses and coordinates for a protein sequence and enzyme. It does not score or rank digests.

PeptideMass (Wilkins et al. 1997) accepts a UniProt accession and returns neutral average masses with pI. It can use signal-peptide and PTM annotations from UniProt. It does not score or rank digests.

Protein Cleaver (Koulouras and Xu 2025) is an R Shiny application built with `cleaver`. It flags peptides as identifiable or non-identifiable using configurable length and mass windows and includes structure mapping and retention-time prediction. The unrecorded source revision assessed for this comparison used the same cleavage engine as pepVet and a different mass table, including 71.079 Da for alanine compared with pepVet's 71.037 Da. The assessed revision had no GRAVY filter, composite score, verdict, or workflow presets.
...
Six presets adjust the length range, GRAVY range, and scoring weights. The figure reports how those settings change valid-peptide counts for BSA, H3.1, and BACE1.
...
Within these rows, the targeted preset keeps the fewest valid peptides and the FFPE preset keeps the most. The BACE1 label changes from `Moderate` under the standard preset at 0.573 to `Good` under `ffpe_degraded` at 0.655. The label change reflects different scoring configurations.

### Pipeline with PeptideRanger

pepVet ranks enzyme-digest combinations and identifies peptides inside the active validity windows. PeptideRanger scores peptides for predicted MS suitability. The combined example therefore compares a digest-level filter with a peptide-level model.

The analysis calculates the difference between mean PeptideRanger scores for pepVet-valid and pepVet-invalid peptides across 25 protein-enzyme combinations. A positive value means that the valid set has the higher mean score in that combination.

```{r pr-fig, eval=has_ggplot2, fig.width=9, fig.height=6, fig.cap="PeptideRanger score difference: mean PR(valid) minus mean PR(invalid). Positive values (green) mean the pepVet-valid set has a higher mean PeptideRanger score. The difference is positive in 17 of 24 computable combinations. One combination has no valid peptides.", fig.alt="Horizontal bar chart of PeptideRanger score differences for 24 computable protein-enzyme combinations, colored by sign."}
...
ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "PeptideRanger score difference by pepVet validity filter",
...
The score difference is positive in 17 of 24 computable combinations. The Lysozyme C and Glu-C combination has no valid peptides, so its difference is undefined. The three largest positive differences are BACE1 with Asp-N and ubiquitin fusion with Glu-C at +0.24, followed by H3.1 with trypsin at +0.21. Six of the seven negative differences have fewer than 10 valid peptides, so their group means rely on small sets.
...
Under the simulated rule, every pepVet-valid peptide is also PC-identifiable. The Protein Cleaver rule accepts additional peptides because it has no GRAVY filter. The exact result depends on the simulated settings and does not replace a run of the external application.
